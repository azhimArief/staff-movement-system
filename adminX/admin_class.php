...
Class Action {
	...
	function login(){
			...
			extract($_POST);		
			$qry = $this->db->query("SELECT * FROM users2 where username = '".$username."' and password = '".$password."' ");
			if($qry->num_rows > 0){
				foreach ($qry->fetch_array() as $key => $value) {
					if($key != 'password' && !is_numeric($key))
						$_SESSION['adminXLogin_'.$key] = $value;
				}
				if($_SESSION['adminXLogin_type'] != 1){
	...
	function login2(){
			...
			if(isset($email))
				$username = $email;
		$qry = $this->db->query("SELECT * FROM users2 where username = '".$username."' and password = '".md5($password)."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
